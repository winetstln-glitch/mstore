    ...
    public function kioskScan(Request $request)
    {
        ...
        if (!$todayAttendance) {
            ...
            if (!$allowAfterCutoff && $this->attendanceService->isPastCutoffTime($shiftCutoff)) {
                return response()->json([
                    'success' => false,
                    'message' => __("Anda tidak bisa melakukan absen sudah melewati batas toleransi absen jam :cutoff WIB. Status kehadiran Anda akan dicatat sebagai Alpha.", ['cutoff' => $shiftCutoff]),
                ], 422);
            }
