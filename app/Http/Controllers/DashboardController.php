...
class DashboardController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // Filter Logic: Exclude HQ / Management roles from region filtering
        $bypassRegionRoles = [
            'super-admin', 'manager',
            'admin', 'direktur',
            'finance', 'hrd-manager', 'hr',
            'operations', 'leader', 'noc-operator',
            'bu-owner', 'manager',
        ];
        if (! $user->hasAnyRole($bypassRegionRoles)) {
        ...
        $networkMonitor = app(\Modules\Network\Services\MonitoringService::class);
        $mixRadiusOk = $networkMonitor->health()['ok'] ?? false;
