...
class IntegrationController extends Controller
{
    ...
    protected function getMikrotikStatus(Request $request)
    {
        ...
        try {
            $isConnected = $this->monitoringService->isRouterConnected($router);
            if (! $isConnected) {
                return response()->json(['error' => 'Could not connect to Mikrotik'], 500);
            }

            $resource = $this->monitoringService->getSystemResource($router);
            $pppoeCount = $this->monitoringService->getPppoeActiveCount($router);
            $hotspotCount = $this->monitoringService->getHotspotActiveCount($router);
