...
class NetworkDiagnosticService
{
    ...
    public function __construct(GenieACSService $genieacs, MonitoringService $monitoringService)
    {
    ...
    public function runDiagnostic(Customer $customer, string $initialMessage = ''): NetworkDiagnostic
    {
        ...
        // Step 4: Check PPPoE Status via MonitoringService
    ...
    protected function checkPppoeStatus(Customer $customer): array
    {
        ...
        try {
            $activeSessions = $this->monitoringService->getPppoeActiveList($router);
            ...
            Log::error("Diagnostic: MonitoringService error for customer {$customer->id}: " . $e->getMessage());
        }
