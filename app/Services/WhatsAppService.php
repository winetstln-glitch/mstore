...
class WhatsAppService
{
    ...
    public function sendMessageWithMediaUrl($phone, $message, $mediaUrl, $mediaType = 'image', $category = 'general', $customerId = null): array
    {
        ...
        try {
            ...
            $providerMessageId = $this->extractProviderMessageId($responseJson);
    ...
    public function sendMessageWithMedia($phone, $message, $mediaBinary, $mediaFilename = 'receipt.png', $category = 'general', $customerId = null): array
    {
        ...
        try {
            ...
            // Log to WhatsAppLog for media messages too
            $whatsappLog = WhatsAppLog::logMessage('outgoing', $phone, $message, 'pending');
    ...
    private function validateProviderResponse($jsonBody, string $rawBody): array
    {
        ...
        if (array_key_exists('status', $jsonBody)) {
            ...
            if (is_string($status)) {
                ...
                if (in_array($statusLower, ['failed', 'error', 'gagal', 'disconnected'])) {
                    $reason = $jsonBody['reason'] ?? $jsonBody['message'] ?? $rawBody;
                    return ['ok' => false, 'error' => (string) $reason];
                }
                // For queued/sent/delivered/read, it's ok (but not yet delivered)
                if (in_array($statusLower, ['queued', 'sent', 'delivered', 'read', 'pending', 'success', 'true', '1'])) {
                    // Ok, but maybe not yet delivered
                }
            } else {
                // Boolean status
     ...
     */
    private function extractProviderMessageId($jsonBody): ?string
    {
        if (! is_array($jsonBody)) {
            return null;
        }

        $idKeys = ['id', 'message_id', 'messageId', 'provider_message_id', 'msgId', 'msg_id', 'reference_id', 'referenceId'];
        foreach ($idKeys as $key) {
            if (isset($jsonBody[$key]) && (is_string($jsonBody[$key]) || is_numeric($jsonBody[$key]))) {
                return (string) $jsonBody[$key];
            }
        }

        // Check nested data
        if (isset($jsonBody['data']) && is_array($jsonBody['data'])) {
            foreach ($idKeys as $key) {
                if (isset($jsonBody['data'][$key]) && (is_string($jsonBody['data'][$key]) || is_numeric($jsonBody['data'][$key]))) {
                    return (string) $jsonBody['data'][$key];
                }
            }
        }
