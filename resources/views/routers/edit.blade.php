                ...
                <form method="POST" action="{{ route('routers.update', $router) }}">
                    ...
                    <div class="row g-3 mb-4">
                        <div class="col-md-6">
                            ...
                            <input type="text" id="name" name="name" value="{{ old('name', $router->name) }}" class="form-control @error('name') is-invalid @enderror" required autofocus>
                            @error('name')
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="text" id="location" name="location" value="{{ old('location', $router->location) }}" class="form-control @error('location') is-invalid @enderror" placeholder="{{ __('Contoh: Kantor Pusat / POP Gedung A') }}">
                            @error('location')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="region_id" class="form-select @error('region_id') is-invalid @enderror" required>
                                ...
                                @foreach($regions as $region)
                                    <option value="{{ $region->id }}" {{ old('region_id', $router->region_id ?? $defaultRegionId ?? '') == $region->id ? 'selected' : '' }}>
                                        {{ $region->name }}
                                    </option>
                                @endforeach
                            </select>
                            ...
                            <input type="text" class="form-control"
                                   value="{{ $regions->firstWhere('id', old('region_id', $router->region_id ?? $defaultRegionId ?? ''))?->name ?? '-' }}" readonly>
                            <input type="hidden" name="region_id" value="{{ old('region_id', $router->region_id ?? $defaultRegionId ?? '') }}">
                            @endif
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="company_id" class="form-select @error('company_id') is-invalid @enderror" required>
                                ...
                                @foreach($companies as $company)
                                    <option value="{{ $company->id }}" {{ old('company_id', $router->company_id ?? $defaultCompanyId ?? '') == $company->id ? 'selected' : '' }}>
                                        {{ $company->name }}
                                    </option>
                                @endforeach
                            </select>
                            @else
                            <input type="text" class="form-control"
                                   value="{{ isset($scopeCompany) && $scopeCompany ? $scopeCompany->name : (isset($companies) ? $companies->firstWhere('id', old('company_id', $router->company_id ?? $defaultCompanyId ?? ''))?->name : '-') }}" readonly>
                            <input type="hidden" name="company_id" value="{{ old('company_id', $router->company_id ?? $defaultCompanyId ?? '') }}">
                            @endif
                        ...
                        <div class="col-md-4">
                            ...
                            <input type="text" id="host" name="host" value="{{ old('host', $router->host) }}" class="form-control @error('host') is-invalid @enderror" required>
                            @error('host')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="col-md-2">
                            ...
                            <input type="number" id="port" name="port" value="{{ old('port', $router->port) }}" class="form-control @error('port') is-invalid @enderror" required>
                            @error('port')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="col-md-3">
                            ...
                            <input type="text" id="username" name="username" value="{{ old('username', $router->username) }}" class="form-control @error('username') is-invalid @enderror" required>
                            @error('username')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="col-md-3">
                            <label for="password" class="form-label">{{ __('Password (kosongkan jika tidak diubah)') }}</label>
                        ...
                        <div class="col-12">
                            <div class="form-check mt-2">
                                <input type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $router->is_active) ? 'checked' : '' }} class="form-check-input">
                    ...
                    <div class="row g-3 mb-4">
                        <div class="col-md-6">
                            ...
                            <input type="text" id="latitude" name="latitude" value="{{ old('latitude', $router->latitude) }}" class="form-control @error('latitude') is-invalid @enderror" placeholder="-6.200000">
                            @error('latitude')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="text" id="longitude" name="longitude" value="{{ old('longitude', $router->longitude) }}" class="form-control @error('longitude') is-invalid @enderror" placeholder="106.816666">
                            @error('longitude')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="col-12">
                            ...
                            <textarea id="description" name="description" rows="3" class="form-control @error('description') is-invalid @enderror">{{ old('description', $router->description) }}</textarea>
                            @error('description')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="d-flex justify-content-end gap-2 border-top pt-4">
                        ...
                        <button type="submit" class="btn btn-success">
                            <i class="fa-solid fa-save me-1"></i> {{ __('Update Router') }}
                        </button>
                    </div>
                </form>
            </div>
