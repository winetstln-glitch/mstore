...
@section('title', __('Tambah Booking CCTV'))

@section('content')
<div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center mb-3">
        <h4 class="mb-0">Tambah Booking CCTV</h4>
        <a href="{{ route('cctv.bookings.index') }}" class="btn btn-outline-secondary">Kembali</a>
    </div>

    <div class="card border-0 shadow-sm">
        <div class="card-body">
            <form method="POST" action="{{ route('cctv.bookings.store') }}">
                @csrf
                <div class="row g-3">
                    <div class="col-md-6">
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="cctv_package_id" class="form-select" required>
                            @foreach($packages as $p)
                                <option value="{{ $p->id }}" {{ old('cctv_package_id') == $p->id ? 'selected' : '' }}>
