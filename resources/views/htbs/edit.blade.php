    ...
    <div class="col-md-8">
        <div class="card shadow-sm border-0 border-top border-4 border-primary">
            <div class="card-header bg-body-tertiary py-3">
                <h5 class="mb-0 fw-bold text-body-emphasis">{{ __('Edit HTB') }}</h5>
            </div>

            <div class="card-body">
                <form action="{{ route('htbs.update', $htb) }}" method="POST">
                    @csrf
                    @method('PUT')

@php
    $uplinkType = old('uplink_type', $htb->parent_htb_id ? 'htb' : 'odp');
    $uplinkId = old('uplink_id', $htb->parent_htb_id ?? $htb->odp_id);
@endphp
                    ...
                    <div class="mb-3">
                        ...
                        <div class="d-flex gap-3">
                            <div class="form-check">
                                <input class="form-check-input" type="radio" name="uplink_type" id="uplink_odp" value="odp" {{ $uplinkType == 'odp' ? 'checked' : '' }} onchange="toggleUplink()">
                                <label class="form-check-label" for="uplink_odp">
                                    {{ __('From ODP (Server)') }}
                                </label>
                            </div>
                            <div class="form-check">
                                <input class="form-check-input" type="radio" name="uplink_type" id="uplink_htb" value="htb" {{ $uplinkType == 'htb' ? 'checked' : '' }} onchange="toggleUplink()">
                    ...
                    <div class="mb-3" id="odp_select_group">
                        ...
                        <select class="form-select @error('uplink_id') is-invalid @enderror" id="odp_id" name="uplink_id">
                            ...
                            @foreach($odps as $odp)
                                <option value="{{ $odp->id }}" {{ $uplinkType == 'odp' && $uplinkId == $odp->id ? 'selected' : '' }}>
                                    {{ $odp->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-3 d-none" id="htb_select_group">
                        ...
                        <select class="form-select @error('uplink_id') is-invalid @enderror" id="parent_htb_id" name="uplink_id" disabled>
                            ...
                            @foreach($parentHtbs as $phtb)
                                <option value="{{ $phtb->id }}" {{ $uplinkType == 'htb' && $uplinkId == $phtb->id ? 'selected' : '' }}>
                                    {{ $phtb->name }} ({{ $phtb->odp ? $phtb->odp->name : 'No ODP' }})
                                </option>
                            @endforeach
                        </select>
                         @error('uplink_id')
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control @error('name') is-invalid @enderror" id="name" name="name" value="{{ old('name', $htb->name) }}" placeholder="{{ __('Leave blank to auto-generate') }}">
                        @error('name')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <div class="row g-2">
                            <div class="col-md-6">
                                <input type="text" class="form-control @error('latitude') is-invalid @enderror" id="latitude" name="latitude" value="{{ old('latitude', $htb->latitude) }}" placeholder="Latitude">
                                @error('latitude')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            <div class="col-md-6">
                                <input type="text" class="form-control @error('longitude') is-invalid @enderror" id="longitude" name="longitude" value="{{ old('longitude', $htb->longitude) }}" placeholder="Longitude">
                                @error('longitude')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="number" class="form-control @error('capacity') is-invalid @enderror" id="capacity" name="capacity" value="{{ old('capacity', $htb->capacity) }}">
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="color" class="form-control form-control-color w-100 @error('color') is-invalid @enderror" id="color" name="color" value="{{ old('color', $htb->color) }}">
                            @error('color')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control @error('description') is-invalid @enderror" id="description" name="description" rows="3">{{ old('description', $htb->description) }}</textarea>
                        @error('description')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="d-flex justify-content-end gap-2">
                        ...
                        <button type="submit" class="btn btn-primary">{{ __('Update HTB') }}</button>
                    </div>
                </form>
            </div>
        </div>
