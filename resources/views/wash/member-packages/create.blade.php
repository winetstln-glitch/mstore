...
@section('title', 'Tambah Paket Member')
...
<div class="container-fluid p-0">
    <div class="d-flex align-items-center mb-3">
        ...
        <h4 class="mb-0 fs-5">Tambah Paket Member</h4>
    </div>

    <div class="card shadow-sm border-0">
        <div class="card-body">
            <form action="{{ route('wash.member-packages.store') }}" method="POST">
                @csrf
                <div class="row g-3">
                    <div class="col-md-6">
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="type" class="form-select @error('type') is-invalid @enderror" required>
                            <option value="wash" {{ old('type') == 'wash' ? 'selected' : '' }}>Wash Saja</option>
                            <option value="wifi" {{ old('type') == 'wifi' ? 'selected' : '' }}>WiFi Saja</option>
                            <option value="both" {{ old('type') == 'both' ? 'selected' : '' }}>Kombinasi (Wash + WiFi)</option>
                        </select>
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="number" name="duration_days" class="form-control @error('duration_days') is-invalid @enderror" value="{{ old('duration_days', 30) }}" min="1">
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="hotspot_profile_id" class="form-select @error('hotspot_profile_id') is-invalid @enderror">
                            ...
                            <option value="{{ $profile->id }}" {{ old('hotspot_profile_id') == $profile->id ? 'selected' : '' }}>{{ $profile->name }}</option>
                            @endforeach
                        </select>
