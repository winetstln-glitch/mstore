            ...
            <div class="card-body p-4">
                <form method="POST" action="{{ route('tickets.store') }}">
                    ...
                    <div class="row g-3 mb-4">
                        ...
                        <div class="col-12" id="manual-customer-name-section" style="display: none;">
                            <label for="manual_customer_name" class="form-label">{{ __('Nama Pelanggan') }} <span class="text-danger">*</span></label>
                            <input type="text" class="form-control" id="manual_customer_name" name="manual_customer_name" value="{{ old('manual_customer_name') }}" placeholder="{{ __('Nama pelanggan (jika tidak terdaftar)') }}">
                        ...
                        <div class="col-12">
                            ...
                            <textarea name="address" id="address" rows="2" placeholder="{{ __('Alamat lengkap jika berbeda dengan alamat pelanggan') }}" class="form-control @error('address') is-invalid @enderror">{{ old('address') }}</textarea>
                            @error('address')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
...
<script>
    $(document).ready(function() {
        ...
        const manualCustomerNameSection = $('#manual-customer-name-section');
        const customerSelect = $('#customer_id');
        const newCustomerInputs = $('#new_customer_name, #new_customer_address');
        const manualCustomerNameInput = $('#manual_customer_name');
        ...
        function toggleCustomerForm() {
            if (typeSelect.val() === 'pasang_baru') {
                ...
                manualCustomerNameSection.hide();
                customerSelect.prop('required', false);
                newCustomerInputs.prop('required', true);
                manualCustomerNameInput.prop('required', false);
            } else if (typeSelect.val() === 'perbaikan') {
                // Untuk perbaikan: bisa pilih existing customer ATAU isi nama manual
                existingCustomerSection.show();
                newCustomerSection.hide();
                manualCustomerNameSection.show();
                // Tidak ada yang required, tapi user harus isi salah satu
                customerSelect.prop('required', false);
