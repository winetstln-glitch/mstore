...
@section('title', __('Edit Booking Wedding'))

@section('content')
<div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center mb-3">
        <h4 class="mb-0">Edit Booking Wedding</h4>
        <a href="{{ route('wedding.bookings.show', $booking) }}" class="btn btn-outline-secondary">Kembali</a>
    </div>

    <div class="card border-0 shadow-sm">
        <div class="card-body">
            <form method="POST" action="{{ route('wedding.bookings.update', $booking) }}">
                @csrf
                @method('PUT')
                <div class="row g-3">
                    <div class="col-md-6">
                    ...
                    <div class="col-md-4">
                        ...
                        <input name="event_date" type="date" class="form-control" value="{{ old('event_date', $booking->event_date?->toDateString()) }}" required>
                    </div>
                    <div class="col-md-8">
                        <label class="form-label">Lokasi</label>
                        <input name="location" class="form-control" value="{{ old('location', $booking->location) }}" required>
                    </div>
                    <div class="col-md-6">
                        <label class="form-label">Paket</label>
                        <select name="wedding_package_id" class="form-select" required>
                            @foreach($packages as $p)
                                <option value="{{ $p->id }}" {{ old('wedding_package_id', $booking->wedding_package_id) == $p->id ? 'selected' : '' }}>
