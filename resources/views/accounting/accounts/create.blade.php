...
<div class="container-fluid py-3">
    <div class="card shadow-sm">
        <div class="card-header">
            <h5 class="mb-0 font-weight-bold text-primary">Tambah Akun</h5>
        </div>
        <div class="card-body">
            <form method="post" action="{{ route('accounting.accounts.store') }}" class="row g-3">
                @csrf
                <div class="col-md-4">
                    <label class="form-label fw-bold small text-muted">Kode Akun</label>
                    <input type="text" name="code" value="{{ old('code') }}" class="form-control @error('code') is-invalid @enderror" placeholder="Contoh: 1101">
                    @error('code')
                ...
                <div class="col-md-8">
                    ...
                    <input type="text" name="name" value="{{ old('name') }}" class="form-control @error('name') is-invalid @enderror" placeholder="Contoh: Kas di Tangan">
                    @error('name')
                    <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
