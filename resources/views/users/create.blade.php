        ...
        <div class="col-md-10">
            <div class="card shadow-sm border-0 border-top border-4 border-primary mb-4">
                <div class="card-header py-3 bg-white">
                    <h5 class="mb-0 fw-bold"><i class="fa-solid fa-user-plus me-2"></i> {{ __('Tambah Pengguna Baru') }}</h5>
                </div>
                            ...
                            <div class="card-body">
                                <div class="row g-3">
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <input type="text" name="job_title" id="job_title" value="{{ old('job_title') }}" class="form-control @error('job_title') is-invalid @enderror" maxlength="255" placeholder="{{ __('contoh: Team Leader Teknisi') }}">
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <select name="company_branch_id" id="company_branch_id" class="form-select @error('company_branch_id') is-invalid @enderror">
                                            ...
                                            @foreach($branches as $branch)
                                                <option value="{{ $branch->id }}" {{ old('company_branch_id') == $branch->id ? 'selected' : '' }}>{{ $branch->name }}</option>
                                            @endforeach
                                        </select>
                                            ...
                                            @foreach($buOptions as $buCode => $buLabel)
                                                <div class="form-check form-check-inline m-0">
                                                    <input class="form-check-input shadow-sm" type="checkbox" name="bu[]" id="bu_{{ $buCode }}" value="{{ $buCode }}"
                                                        {{ is_array(old('bu')) && in_array($buCode, old('bu')) ? 'checked' : '' }}>
                                    ...
                                    <div class="col-md-4 mt-3">
                                        ...
                                        <select name="department" id="department" class="form-select @error('department') is-invalid @enderror">
                                            ...
                                            @foreach($departmentOptions as $deptCode => $deptLabel)
                                                <option value="{{ $deptCode }}" {{ old('department') === $deptCode ? 'selected' : '' }}>{{ $deptLabel }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="col-md-4 mt-3">
                                        ...
                                        <select name="bu_level" id="bu_level" class="form-select @error('bu_level') is-invalid @enderror">
                                            ...
                                            @foreach($buLevelOptions as $lv)
                                                <option value="{{ $lv }}" {{ old('bu_level') === $lv ? 'selected' : '' }}>{{ ucwords(str_replace(['-', '_'], ' ', $lv)) }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="col-md-4 mt-3">
                                        ...
                                        <select name="field_level" id="field_level" class="form-select @error('field_level') is-invalid @enderror">
                                            ...
                                            @foreach($fieldLevelOptions as $flCode => $flLabel)
                                                <option value="{{ $flCode }}" {{ old('field_level') === $flCode ? 'selected' : '' }}>{{ $flLabel }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                            ...
                            <div class="card-body">
                                <div class="row g-3">
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <input type="number" name="monthly_salary" id="monthly_salary" value="{{ old('monthly_salary') }}" class="form-control @error('monthly_salary') is-invalid @enderror" min="0">
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <input type="number" name="daily_salary" id="daily_salary" value="{{ old('daily_salary') }}" class="form-control @error('daily_salary') is-invalid @enderror" min="0">
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        if (companySelect && branchSelect) {
            ...
            const selectedOldBranch = @json(old('company_branch_id'));
