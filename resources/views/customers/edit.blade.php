        ...
        <div class="card shadow-sm border-0 border-top border-4 border-primary">
            ...
            <div class="card-body p-3 p-md-4">
                <form method="POST" action="{{ route('customers.update', $customer) }}" id="editCustomerForm">
                    ...
                    <!-- Coordinator first (filter for region) -->
    <div class="row g-3 mb-4">
        <div class="col-md-6">
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const closureSelect = document.getElementById('closure_id');
        let initialRegionId = @json(optional($customer->odp)->region_id ?? null);
        
        if (coordinatorSelect) {
            // Pilih coordinator awal berdasarkan region yang cocok.
            if (initialRegionId) {
                const toSelect = Array.from(coordinatorSelect.options).find(opt => String(opt.dataset.regionId || opt.getAttribute('data-region-id')) === String(initialRegionId));
                if (toSelect) {
                    coordinatorSelect.value = toSelect.value;
                    currentRegionId = toSelect.dataset.regionId || toSelect.getAttribute('data-region-id');
                }
            }
        ...
        if (closureSelect) {
            ...
            if (currentOdcId) {
                ...
                if (toSelect) {
                    closureSelect.value = toSelect.value;
                }
            }
