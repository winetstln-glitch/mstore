    ...
    <div class="col-12 col-lg-10 px-3 px-lg-0">
        <div class="card shadow-sm border-0 border-top border-4 border-primary">
            <div class="card-header  py-3 d-flex justify-content-between align-items-center">
                <h5 class="mb-0 fw-bold text-body-emphasis fs-6 fs-md-5">{{ __('Tambah Pelanggan') }}</h5>
                ...
                <form method="POST" action="{{ route('customers.store') }}" id="customerForm">
                    ...
                    <div class="row g-3 mb-4">
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="text" name="name" id="name" value="{{ old('name', $prefill['name'] ?? '') }}" required class="form-control @error('name') is-invalid @enderror" placeholder="{{ __('Masukkan nama lengkap') }}">
                            @error('name')
                        ...
                        <div class="col-12">
                            ...
                            <textarea name="address" id="address" rows="2" class="form-control @error('address') is-invalid @enderror" placeholder="{{ __('Masukkan alamat lengkap') }}">{{ old('address') }}</textarea>
                            @error('address')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="col-12">
                            <h6 class="fw-bold text-body-secondary text-uppercase small mt-3 mb-3 border-top pt-3">{{ __('Akun Portal (Opsional)') }}</h6>
                            <div class="row g-3">
                                ...
                                <div id="portal_fields" class="{{ old('create_user') ? '' : 'd-none' }}">
                                    <div class="row g-3">
                                        ...
                                        <div class="col-md-4">
                                            <label for="password" class="form-label small text-muted fw-bold">{{ __('Password Portal') }}</label>
                        ...
                        <script>
                            ...
                            function togglePortalFields() {
                                ...
                                // Nonaktifkan input saat disembunyikan agar tidak mengganggu validasi, dan bersihkan isinya.
                        ...
                        <div class="col-12">
                            <div class="form-text text-muted mb-2 small">{{ __('Ketuk peta untuk menentukan lokasi') }}</div>
                    ...
                    <div class="row g-3 mb-4">
                        ...
                        <!-- PPPoE Username -->
                        <div class="col-6">
                            <label for="pppoe_user" class="form-label small text-muted fw-bold">{{ __('User PPPoE') }}</label>
                        ...
                        <!-- PPPoE Password -->
                        <div class="col-6">
                            <label for="pppoe_password" class="form-label small text-muted fw-bold">{{ __('Password PPPoE') }}</label>
                        ...
                        <!-- OLT -->
                        <div class="col-md-6">
                            <label for="olt_id" class="form-label small text-muted">{{ __('Server OLT') }}</label>
                        ...
                        <div class="col-md-6">
                            <label for="coordinator_id" class="form-label small text-muted fw-bold">{{ __('Pengurus/Coordinator') }}</label>
                            <select id="coordinator_id" class="form-select">
                                <option value="">{{ __('Pilih Pengurus (opsional)') }}</option>
                                @foreach(($coordinators ?? []) as $coordinator)
                                    <option value="{{ $coordinator->id }}" data-region-id="{{ $coordinator->region_id }}">{{ $coordinator->name }} {{ $coordinator->region ? '(' . $coordinator->region->name . ')' : '' }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="col-12">
                            <label class="form-label d-block small text-muted fw-bold">{{ __('Tipe Koneksi') }}</label>
                            <div class="d-flex gap-3 align-items-center">
                                <div class="form-check">
                                    ...
                                    <label class="form-check-label" for="conn_odp">{{ __('Langsung ke ODP') }}</label>
                                </div>
                                <div class="form-check">
                                    ...
                                    <label class="form-check-label" for="conn_htb">{{ __('Melalui HTB') }}</label>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="status" id="status" class="form-select @error('status') is-invalid @enderror">
                                ...
                                <option value="terminated" {{ old('status') == 'terminated' ? 'selected' : '' }}>{{ __('Berhenti') }}</option>
                            </select>
                            @error('status')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                    ...
                    <div class="d-flex flex-column-reverse flex-md-row justify-content-end gap-2 border-top pt-4 mobile-sticky-footer">
                        ...
                        <button type="submit" class="btn btn-primary w-100 w-md-auto px-4">{{ __('Simpan Pelanggan') }}</button>
                    </div>
                </form>
            </div>
        </div>
...
<script>
    ...
    function filterByCoordinator(coordinatorId) {
        const coordinatorList = @json($coordinators ?? []);
        let selectedCoordinator = null;
        if (coordinatorId) {
            selectedCoordinator = coordinatorList.find(c => String(c.id) === String(coordinatorId));
        }
        currentRegionId = selectedCoordinator ? selectedCoordinator.region_id : null;
        filterOdpAndHtb();
    }

    function filterOdpAndHtb() {
    ...
    function filterOdpByClosure(closureId) {
        ...
        // Coba ambil nama ODC dari data yang tersedia.
        ...
        let list = odpList;
        if (currentRegionId) {
            list = list.filter(o => String(o.region_id) === String(currentRegionId));
        }
        if (selectedOdcId) {
            list = list.filter(o => String(o.odc_id) === String(selectedOdcId));
        }
        ...
        // Pertahankan pilihan jika masih ada setelah difilter.
    ...
    function filterHtb() {
        const htbList = @json($htbs ?? []);
        const htbSelect = document.getElementById('htb_id');
        const currentValue = htbSelect.value;

        htbSelect.innerHTML = '';
        const placeholder = document.createElement('option');
        placeholder.value = '';
        placeholder.textContent = '-- {{ __('Pilih HTB') }} --';
        htbSelect.appendChild(placeholder);

        let list = htbList;
        if (currentRegionId) {
            list = list.filter(htb => htb.odp && String(htb.odp.region_id) === String(currentRegionId));
        }

        list.forEach(htb => {
            const opt = document.createElement('option');
            opt.value = htb.id;
            const cap = (htb.capacity !== null && htb.capacity !== undefined) ? htb.capacity : '∞';
            let htbName = htb.name;
            if (htb.parent) {
                htbName += ` (via ${htb.parent.name})`;
            }
            opt.textContent = `${htbName} (${htb.filled}/${cap})${(htb.capacity !== null && htb.filled >= htb.capacity) ? ' - Penuh' : ''}`;
            if (htb.capacity !== null && htb.filled >= htb.capacity) opt.disabled = true;
            htbSelect.appendChild(opt);
        });

        const stillExists = Array.from(htbSelect.options).some(opt => opt.value === currentValue);
        if (stillExists) {
            htbSelect.value = currentValue;
        } else {
            htbSelect.value = '';
        }
    }
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (coordinatorSelect) {
            coordinatorSelect.addEventListener('change', function() {
                filterByCoordinator(this.value);
            });
        }
